...
class FH_UltimateBot(ctk.CTk):
    ...
    def find_image_with_verify_box(
        self,
        template_path,
        verify_box,
        region=None,
        ...
        try:
            ...
            scales_to_try = self.get_scales_to_try(fast_mode=fast_mode)
            best_main_score = 0.0
            best_verify_score = 0.0

            for scale in scales_to_try:
                tpl_c, _ = self.get_scaled_template(template_path, scale)
                if tpl_c is None:
                    continue
                ...
                x1 = max(0, min(w - 1, int(w * verify_box[0])))
                y1 = max(0, min(h - 1, int(h * verify_box[1])))
                x2 = max(x1 + 1, min(w, int(w * verify_box[2])))
                y2 = max(y1 + 1, min(h, int(h * verify_box[3])))
                tpl_verify = tpl_c[y1:y2, x1:x2]

                res = cv2.matchTemplate(screen_bgr, tpl_c, cv2.TM_CCOEFF_NORMED)
                _, scale_main_score, _, _ = cv2.minMaxLoc(res)
                best_main_score = max(best_main_score, float(scale_main_score))

                ys, xs = np.where(res >= main_threshold)
                if len(xs) == 0:
                    continue

                candidates = [(float(res[y, x]), x, y) for x, y in zip(xs, ys)]
                ...
                for main_score, x, y in candidates:
                    ...
                    if roi.shape[:2] != tpl_c.shape[:2]:
                        continue

                    roi_verify = roi[y1:y2, x1:x2]
                    verify_score = self.match_template_score(roi_verify, tpl_verify)
                    best_verify_score = max(best_verify_score, float(verify_score))

                    if verify_score >= verify_threshold:
                        pos = (
                            x + w // 2 + (region[0] if region else 0),
                            y + h // 2 + (region[1] if region else 0),
                        )
                        self.last_positions[template_path] = pos
                        self.log(
                            f"[verify_box] 命中 {template_path}, "
                            f"main: {main_score:.3f}, verify: {verify_score:.3f}"
                        )
                        return pos

            self.log(
                f"[verify_box] 未命中 {template_path}, "
                f"main最高: {best_main_score:.3f}, verify最高: {best_verify_score:.3f}"
            )
            return None

        except Exception as e:
            self.log(f"find_image_with_verify_box 异常: {e}")
            return None

    def find_selected_car_card_region(self, region=None):
        """
        通过黄绿色选中边框定位当前选中的车辆卡片。
        返回绝对屏幕坐标区域 (x, y, w, h)，后续识图只在这张卡片内进行。
        """
        base_region = region or self.regions["全界面"]
        screen_bgr = self.capture_region(base_region)
        screen_h, screen_w = screen_bgr.shape[:2]

        # 只看车辆网格主体，避开左侧详情栏、顶部货币和底部提示文字。
        crop_x = int(screen_w * 0.18)
        crop_y = int(screen_h * 0.15)
        crop_w = int(screen_w * 0.76)
        crop_h = int(screen_h * 0.75)
        grid_bgr = screen_bgr[crop_y:crop_y + crop_h, crop_x:crop_x + crop_w]

        hsv = cv2.cvtColor(grid_bgr, cv2.COLOR_BGR2HSV)
        lower = np.array([20, 80, 120])
        ...
        for contour in contours:
            ...
            if w >= 120 and h >= 100 and area >= 18000 and 0.8 <= ratio <= 2.4:
        ...
        x = max(0, x - pad)
        y = max(0, y - pad)
        w = min(crop_w - x, w + pad * 2)
        h = min(crop_h - y, h + pad * 2)

        base_x, base_y = base_region[0], base_region[1]
        return (base_x + crop_x + x, base_y + crop_y + y, w, h)

    def wait_for_old_cc(self, region=None, timeout=1.0, interval=0.2, fast_mode=True, return_match=False):
        start = time.time()

        while self.is_running and time.time() - start < timeout:
            selected_region = self.find_selected_car_card_region(region)
            if not selected_region:
                self.sleep_while_running(interval)
                continue

            # 只在当前黄框卡片内识别，并二次校验右下角等级区域。
            # oldCC.png 的右下角是 B 600；S2 900 虽然车身相同，也不能通过 verify_box。
            pos = self.find_image_with_verify_box(
                "oldCC.png",
                region=selected_region,
                verify_box=(0.70, 0.76, 0.99, 0.98),
                main_threshold=0.60,
                verify_threshold=0.90,
                fast_mode=fast_mode
            )
            if pos:
                return (pos, "oldCC.png") if return_match else pos

            self.sleep_while_running(interval)

        return (None, None) if return_match else None
    ...
    def find_image_with_element_multi(self, main_path, sub_path, region=None, fast_mode=True,
        ...
        try:
            ...
            for scale in scales_to_try:
                ...
                res_main = cv2.matchTemplate(screen_bgr, main_tpl_c, cv2.TM_CCOEFF_NORMED)
                _, scale_main_score, _, _ = cv2.minMaxLoc(res_main)
                best_main_score = max(best_main_score, float(scale_main_score))
                ys, xs = np.where(res_main >= main_threshold)
                if len(xs) == 0:
                    continue
                ...
                for main_score, x, y in candidates:
                    ...
                    best_like_score = max(best_like_score, float(like_score))
            ...
            if best_final_score >= final_threshold:
                self.log(f"[multi_match] 命中 {main_path} 最终分数: {best_final_score:.3f}")
                return best_pos

            self.log(
                f"[multi_match] 未命中 {main_path}，"
                f"main最高: {best_main_score:.3f}, "
                f"like最高: {best_like_score:.3f}, "
                f"final最高: {best_final_score:.3f}"
            )
            return None
    ...
    def sleep_while_running(self, seconds, tick=0.05):
        """等待一段时间，同时保持对停止按钮的响应。"""
        end_time = time.time() + seconds
        while self.is_running and time.time() < end_time:
            time.sleep(min(tick, max(0, end_time - time.time())))
        return self.is_running

    def press_key_repeated(self, key, times, press_delay=0.08, interval=0.0):
        """按固定节奏重复按键；循环中统一检查停止信号。"""
        for _ in range(times):
            if not self.is_running:
                return False
            self.hw_press(key, delay=press_delay)
            if interval > 0 and not self.sleep_while_running(interval):
                return False
        return True

    def press_sequence(self, actions):
        """
        顺序执行按键动作。
        actions 支持 (key, wait_after) 或 (key, press_delay, wait_after)。
        """
        for action in actions:
            ...
            if len(action) == 2:
                key, wait_after = action
                press_delay = 0.08
            else:
                key, press_delay, wait_after = action

            self.hw_press(key, delay=press_delay)
            if wait_after > 0 and not self.sleep_while_running(wait_after):
                return False

        return True

    def click_and_wait(self, pos, wait_after, double=False):
        """点击游戏坐标后等待指定时间；pos 为空时保持 game_click 的原有兜底行为。"""
        self.game_click(pos, double=double)
        return self.sleep_while_running(wait_after)

    def select_brand_from_list(
        self,
        image_list,
        attempts=30,
        threshold=0.75,
        ...
        """
        在品牌筛选列表中寻找指定品牌图标，找不到时向上移动继续扫。
        返回点击到的位置；返回 None 表示未找到或任务已停止。
        """
        scan_region = region or self.regions["全界面"]
        ...
        for _ in range(attempts):
            ...
            if brand_pos:
                if not self.click_and_wait(brand_pos, click_delay):
                    return None
                return brand_pos

            if step_key:
                self.hw_press(step_key)
                if not self.sleep_while_running(step_delay):
                    return None
    ...
    def select_cc_brand(self, attempts=30, fail_log="选品牌失败", click_delay=1.0):
        """选择消耗品车辆品牌；多处流程都会回到这个筛选入口。"""
        return self.select_brand_from_list(
            ["CCbrand.png"],
            attempts=attempts,
            threshold=0.75,
            wait_timeout=0.8,
    ...
    def find_new_consumable_car(self, max_attempts=85, refresh_every=8):
        """
        在车辆列表中寻找带 newcartag 的 CC 车辆。
        每扫 refresh_every 次后重新打开品牌筛选，避免列表焦点漂移导致漏识别。
        """
        for search_attempt in range(max_attempts):
            ...
            pos_target = self.find_image_with_element(
                ...
                fast_mode=False     # 使用完整缩放集合提升复杂界面下的命中率
            )
            if pos_target:
                return pos_target

            if not self.press_key_repeated("right", 4, press_delay=0.05, interval=0.08):
                return None
            if not self.sleep_while_running(0.4):
                return None

            should_refresh_brand = (search_attempt + 1) % refresh_every == 0
            is_last_attempt = search_attempt >= max_attempts - 1
            if should_refresh_brand and not is_last_attempt:
                if not self.press_sequence([("backspace", 1.0)]):
                    return None
                if not self.select_cc_brand(attempts=30, fail_log="选品牌失败", click_delay=1.0):
                    return None

        return None

    def finish_module_early(self, module_name, current_count, target_count, reason):
        """
        当前模块因为游戏界面状态不满足而提前结束。
        返回 True 让流水线进入下一个已配置功能；用户手动停止仍由调用处返回 False。
        """
        self.log(f"{module_name}提前结束：{reason}（进度 {current_count}/{target_count}），继续进入下一个功能。")
        return True
    ...
    def find_template_crop(self, template_path, crop_box, region=None, threshold=0.75, fast_mode=True):
        """把模板的局部区域拿出来匹配，用于复用 oldCC.png 里的 B 600 等小块。"""
        if not self.is_running:
            return None
        ...
        try:
            ...
            for scale in self.get_scales_to_try(fast_mode=fast_mode):
                ...
                h, w = tpl_c.shape[:2]
                x1 = max(0, min(w - 1, int(w * crop_box[0])))
                y1 = max(0, min(h - 1, int(h * crop_box[1])))
                x2 = max(x1 + 1, min(w, int(w * crop_box[2])))
                y2 = max(y1 + 1, min(h, int(h * crop_box[3])))
                tpl_crop = tpl_c[y1:y2, x1:x2]

                ch, cw = tpl_crop.shape[:2]
                if ch < 5 or cw < 5 or ch > screen_bgr.shape[0] or cw > screen_bgr.shape[1]:
                    continue

                res = cv2.matchTemplate(screen_bgr, tpl_crop, cv2.TM_CCOEFF_NORMED)
                _, max_val, _, max_loc = cv2.minMaxLoc(res)
                if max_val >= threshold:
                    return (
                        max_loc[0] + cw // 2 + (region[0] if region else 0),
                        max_loc[1] + ch // 2 + (region[1] if region else 0),
                    )

            return None
        except Exception as e:
            self.log(f"find_template_crop 异常: {e}")
            return None

    def is_current_header_b600_old_cc(self):
        """
        判断左上角当前车辆栏是否显示 B 600。
        这里复用 oldCC.png 右下角等级块，主要用于区分同车型的 B600 和 S2 900。
        """
        gx, gy, gw, gh = self.regions["全界面"]
        header_region = (gx, gy, int(gw * 0.42), int(gh * 0.10))
        return bool(self.find_template_crop(
            "oldCC.png",
            crop_box=(0.70, 0.76, 0.99, 0.98),
    ...
    def logic_race(self, target_count):
        ...
        self.log("切换到创意中心...")
        if not self.press_key_repeated("pagedown", 4, press_delay=0.15, interval=0.3):
            return False
        ...
        if not self.click_and_wait(pos_el, 1.2):
            return False
        ...
        if not self.click_and_wait(pos_yg, 1.5):
            return False

        if not self.press_sequence([
            ("backspace", 0.8),
            ("up", 0.4),
            ("enter", 0.8),
        ]):
        ...
        for char in code_text:
            ...
            if char in DIK_CODES:
                ...
                if not self.sleep_while_running(0.05):
                    return False
        ...
        if not pos_target:
            ...
            if not self.press_sequence([("backspace", 1.2)]):
                return False

            if not self.select_brand_from_list(
                ["skillcarbrand.png"],
                attempts=3,
                threshold=0.75,
                wait_timeout=0.8,
            ...
            for _ in range(200):
                ...
                if not self.press_key_repeated("right", 4, press_delay=0.08, interval=0.08):
                    return False
                if not self.sleep_while_running(0.4):
                    return False
        ...
        if not self.click_and_wait(pos_target, 0.5):
            return False
        if not self.press_sequence([("enter", 4.0)]):
            return False
        ...
        while self.race_counter < target_count:
            ...
            for _ in range(1500):
                ...
                if not self.press_sequence([("down", 0.25)]):
                    return False

            if not pos:
                return self.finish_module_early(
                    "循环跑图",
                    self.race_counter,
                    target_count,
                    "找不到赛事起点"
            ...
            if not self.click_and_wait(pos, 4.0):
                return False
            self.hw_key_down("w")
            ...
            if self.race_counter == target_count - 1:
                if not self.press_sequence([("enter", 2.0)]):
                    return False
            else:
                if not self.press_sequence([
                    ("x", 0.8),
    ...
    def logic_buy_car(self, target_count):
        ...
        if not self.click_and_wait(pos, 0.6, double=True):
            return False
        ...
        if not self.click_and_wait(pos, 1.0, double=True):
            return False

        if not self.press_sequence([("backspace", 0.5)]):
            return False

        if not self.select_cc_brand(attempts=20, fail_log="未找到品牌", click_delay=0.8):
            return False

        if not self.press_sequence([("down", 0.4)]):
            return False
        ...
        if not self.click_and_wait(pos_22b, 1.0, double=True):
            return False
        ...
        if not self.press_key_repeated("esc", 5, interval=0.8):
            return False
    ...
    def logic_super_wheelspin(self, target_count):
        ...
        self.log("进入车辆与收藏...")
        if not self.press_sequence([("pagedown", 0.15, 1.0)]):
            return False
        ...
        if not self.click_and_wait(pos_buycar, 0.8):
            return False
        if not self.press_sequence([("enter", 5)]):
            return False
        ...
        if not self.click_and_wait(pos_bs, 1.0):
            return False
        self.log("进入车辆界面...")
        if not self.press_sequence([("pagedown", 0.15, 0.5)]):
            return False
        ...
        while self.cj_counter < target_count:
            ...
            pos_target = self.find_new_consumable_car(max_attempts=85, refresh_every=5)
            if not pos_target:
                return self.finish_module_early(
                    "超级抽奖",
                    self.cj_counter,
            ...
            if not self.click_and_wait(pos_target, 1.2):
                return False
            if not self.press_sequence([
                ("enter", 1.0),
                ("enter", 1.0),
            ...
            for _ in range(60):
                ...
                if not self.press_sequence([("esc", 0.5)]):
                    return False
            ...
            if not self.click_and_wait(pos_sjy, 0.5):
                return False
            ...
            if not self.click_and_wait(pos_cls, 1.5):
                return False
            ...
            if pos_exp:
                ...
                if not self.sleep_while_running(1.0):
                    return False
                if not self.press_sequence([("enter", 1.5)]):
                    return False
    ...
    def sell_consumable_car(self, target_count):
        ...
        if self.is_current_header_b600_old_cc():
            self.log("当前驾驶车辆为 B600 斯巴鲁，先切换到收藏车避免删除当前车辆")
            if not self.press_sequence([
                ("enter", 2.0),  # 进入我的车辆
                ("y", 1.0),
                ("enter", 0.8),
            ...
            if pos:
                self.log("找到上车，执行点击")
                if not self.click_and_wait(pos, 2.0):
                    return False
            else:
                ...
                if not self.press_sequence([
                    ...
                    return False
            if not self.sleep_while_running(2.0):
                return False
            ...
            if not self.sleep_while_running(1.5):
                return False
        else:
            self.log("当前驾驶车辆不是 B600 斯巴鲁，跳过收藏车切换，直接筛选重复项")
        # 筛选重复项
        if not self.press_sequence([
            ("enter", 2.0),  # 进入我的车辆
            ("y", 0.8)
        ...
        self.move_to_game_coord(5, 5)
        self.log("识别并选择 重复项...")
        pos_duplicate = self.wait_for_image(
            "duplicate.png",
            region=self.regions["全界面"],
            threshold=0.65,
            timeout=10,
        ...
        if not pos_duplicate:
            self.log("未识别到 duplicate.png（重复项），无法继续移除车辆")
            return False
        if not self.click_and_wait(pos_duplicate, 1.0):
            return False
        if not self.press_sequence([("esc", 0.8)]):
            return False

        self.log("开始删除重复项车辆！！！请人工确认是否移除")
        ...
        while self.sc_count < target_count:
            ...
            if not self.press_sequence([("backspace", 1.0)]):
                return False
            self.log("识别并选择 CCbrand...")
            pos_brand = self.select_brand_from_list(
                ["CCbrand.png", "CCbrand_b.png"],
                attempts=1,
                threshold=0.7,
                wait_timeout=1,
                interval=0.3,
                step_key=None,
                click_delay=1.5,
                fail_log="未识别到 CCbrand.png，无法继续移除车辆"
            )
            if not pos_brand:
                return self.finish_module_early(
                    "移除车辆",
            ...
            if not self.is_running:
                return False
            nextPageCount = 0
            while nextPageCount < 2:
                pos_old_cc, _ = self.wait_for_old_cc(
                    region=self.regions["全界面"],
                    timeout=1.0,
                    interval=0.2,
                    fast_mode=True,
                ...
                if not pos_old_cc:
                    self.log("当前黄框车辆未识别到 oldCC.png，跳过当前车辆")
                    if not self.press_sequence([("down", 0.5)]):
                        return False
                    nextPageCount += 1
                    continue
                self.log("当前黄框车辆识别到 oldCC.png，进入删除菜单")
                self.hw_press("enter")
                if not self.sleep_while_running(0.5):
                    return False
                break
            if nextPageCount == 2:
                return self.finish_module_early(
                    ...
                    self.sc_count,
                    target_count,
                    "连续翻页仍未识别到当前黄框车辆为 oldCC.png"
                )
            
            pos_delete = self.wait_for_image(
                "deleteBtn.png",
                region=self.regions["全界面"],
                threshold=0.8,
                timeout=1.0,
            ...
            if not pos_delete:
                return self.finish_module_early(
                    ...
                    self.sc_count,
                    target_count,
                    "识别到 oldCC.png 但未找到 deleteBtn.png"
                )

            self.game_click(pos_delete)
            if not self.sleep_while_running(0.5):
                return False
            # 确认弹窗默认不是“嗯”，需要向下再确认。
            if not self.press_sequence([
        ...
        if not self.press_key_repeated("esc", 3, interval=1.0):
            return False
